...
ngx_http_ruby_code_from_file(ngx_pool_t *pool, ngx_str_t *code_file_path)
{
    ...
    code->code_type = NGX_HTTP_RUBY_CODE_TYPE_FILE;
    return code;
}

ngx_http_ruby_code_t *
ngx_http_ruby_code_from_string(ngx_pool_t *pool, ngx_str_t *code_inline_str)
{
    ngx_http_ruby_code_t *code;
    size_t len;
    ...
    len = ngx_strlen(code_inline_str->data);
    ...
    ngx_cpystrn((u_char *)code->code.string, code_inline_str->data, len + 1);
    code->code_type = NGX_HTTP_RUBY_CODE_TYPE_STRING;
